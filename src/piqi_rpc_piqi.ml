...
and Request:
  sig
    type t = {
      mutable name: string;
      mutable data: Piqi_rpc_piqi.binary option;
    }
  end = Request
...
and gen__request code x =
  ...
  let _data = Piqirun.gen_optional_field 2 gen__binary x.Request.data in
  Piqirun.gen_record code (_name :: _data :: [])
...
and default_rpc_error () = (default_client_error () :> Piqi_rpc_piqi.rpc_error)
