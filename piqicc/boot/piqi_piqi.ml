  ...
  in
    refer count
      (match code with
       ...
       | 365880944 ->
           let res =
             let count = curr_count () in refer count (parse_piqi_any x)
           in `piqi_any res
       | _ -> Piqirun.error_variant x code)
and parse_any x =
  let x = Piqirun.parse_record x in
  ...
  (refer x;
   Piqirun.gen_record code
     [ (match x with
        ...
        | `piqi_any x -> gen__piqi_any 365880944 x) ])
and gen__any code x =
  (refer x;
