...
and
  Extend :
    sig
      type t =
        { mutable name : Piqtype.name list;
...
and parse_extend x =
  ...
  in
    refer count
      (let (_name, x) =
         Piqirun.parse_repeated_field 150958667 parse_name x in
       let (_quote, x) =
         Piqirun.parse_repeated_field 455932215 parse_piq_any x
       in
         (Piqirun.check_unparsed_fields x;
          { Extend.name = _name; Extend.quote = _quote; }))
  ...
  (refer x;
   ...
   in
     Piqirun.gen_record code
       [ _proto_name; _name; _option; _parent; _ocaml_name; _proto_custom;
         _json_name ])
...
and gen__piqi code x =
  (refer x;
   ...
   in
     Piqirun.gen_record code
       [ _modname; _resolved_piqdef; _resolved_import; _extend; _piqdef;
         _import; _included_piqi; _custom_field; _resolved_func; _includ;
         _imported_piqdef; _proto_package; _func; _ocaml_module;
...
and gen__extend code x =
  (refer x;
   let _name =
     Piqirun.gen_repeated_field 150958667 gen__name x.Extend.name in
   let _quote =
     Piqirun.gen_repeated_field 455932215 gen__piq_any x.Extend.quote
   in Piqirun.gen_record code [ _name; _quote ])
...
and default_extend () = { Extend.name = []; Extend.quote = []; }
...
let _ =
  add_embedded_piqi
    ("piqic/piqi-boot-ocaml",
     "\n\n.include [ .module piqi.org/piqi-boot ]\n\n\n.custom-field ocaml-type\n\n\n.extend [\n    (.name int uint)\n    .ocaml-type \"int\"\n]\n\n\n.extend [\n    (.name int32 uint32 int32-fixed uint32-fixed proto-int32)\n    .ocaml-type \"int32\"\n]\n\n\n.extend [\n    (.name int64 uint64 int64-fixed uint64-fixed proto-int64)\n    .ocaml-type \"int64\"\n]\n\n")
